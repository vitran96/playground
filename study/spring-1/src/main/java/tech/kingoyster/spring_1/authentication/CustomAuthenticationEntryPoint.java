...
public class CustomAuthenticationEntryPoint implements AuthenticationEntryPoint {
    ...
    public void commence(
        ...
        if (Objects.nonNull(customAuthException)
                && customAuthException
                        instanceof JWTVerificationException jwtVerificationException) {
            message = jwtVerificationException.getMessage();
        }

        var errorDetail =
                new ErrorDetail(LocalDateTime.now(), 0, message, request.getServletPath(), null);
